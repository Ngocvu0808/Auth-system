package com.ttt.mar.auth.config;
...
public class GenerateUniqueKey {
  ...
  public GenerateUniqueKey(String salt) {
    this(salt, 0);
  }

  public GenerateUniqueKey(String salt, int minHashLength) {
    this(salt, minHashLength, DEFAULT_ALPHABET);
  }

  public GenerateUniqueKey(String salt, int minHashLength, String alphabet) {
    this.salt = salt;
    if (minHashLength < 0) {
  ...
  private String _encode(long... numbers) {
    ...
    if (ret_str.length() < this.minHashLength) {
      guardIndex =
          (numberHashInt + (int) (ret_str.toString().toCharArray()[0])) % this.guards.length();
      ...
      if (ret_str.length() < this.minHashLength) {
        guardIndex =
            (numberHashInt + (int) (ret_str.toString().toCharArray()[2])) % this.guards.length();
        guard = this.guards.toCharArray()[guardIndex];
    ...
    while (ret_str.length() < this.minHashLength) {
      ...
      ret_str = new StringBuilder(
          alphabet.substring(halfLen) + ret_str + alphabet.substring(0, halfLen));
